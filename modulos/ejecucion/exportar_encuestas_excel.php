...
$ejecuciones_tb = $ejeData->getEEncuestas("enc.pla_id = " . $pla_id, false);
$tipoEncuesta = $ejeData->getTipoEncuesta($ejecuciones_tb[0]['id_element']);
...
echo"<tr><th colspan = '".$numero_preguntas."' bgcolor='#CCCCCC'><center>" . $html->traducirTildes(PLANEACION_REPORTE_EXCEL) . "</center></th></tr>";
                ...
                switch ($pb['tipo']) {
                    case 1:
                        ...
                        if ($ejeData->getRespuestaPreguntaDeEncuesta($pb['id'], $enc_id) == '1') {
                            echo "<td>" . $html->traducirTildes( $html->traducirTildes('Si') ). "</td>";
                        } else {
                            echo "<td>" . $html->traducirTildes( ('')) . "</td>";
                        }
                        break;
                    case 2:
                        $respuesta = $ejeData->getOpcionesPreguntas($pb['id'] . ' and ipo_id= ' . $ejeData->getRespuestaPreguntaDeEncuesta($pb['id'], $enc_id));
                        echo "<td>" . $html->traducirTildes( $html->traducirTildes($respuesta[0]['nombre']) ). "</td>";
                        break;
                    case 3:
                        $respuesta = '';
                        ...
                        echo "<td>" . $html->traducirTildes( $html->traducirTildes($respuesta)) . "</td>";
                        break;
                    case 4:
                    case 5:
                    case 7:
                    case 8:
                        echo "<td>" . $html->traducirTildes( $html->traducirTildes($ejeData->getRespuestaPreguntaDeEncuesta($pb['id'], $enc_id))) . "</td>";
                        break;
                    case 6:
                        echo "<td>" . $html->traducirTildes( ('') ). "</td>";
                        break;
