...
/**
 * Modulo Manuales
 * maneja el modulo METODOLOGIAS en union con CManual y CManualData
 *
 * @see CManual
 * @see CManualData
 *
...
$manData = new CMetodologiasData($db);
...
switch ($task) {
    /**
     * la variable list, permite hacer la carga la página con la lista de objetos METODOLOGIAS según los parámetros de entrada
     */
    case 'list':
        $criterio = '1';
        ...
        $titulos = array(METODOLOGIAS_NOMBRE, METODOLOGIAS_ARCHIVO);
        $dt->setDataRows($manuales);
        $dt->setTitleRow($titulos);

        $dt->setTitleTable(TABLA_METODOLOGIAS);
        ...
        break;
    /**
     * la variable add, permite hacer la carga la página con las variables que componen el objeto METODOLOGIAS, ver la clase CManual
     */
    case 'add':
        ...
        $form->setTitle(AGREGAR_METODOLOGIAS);
        $form->setId('frm_add_manual');
        $form->setMethod('post');
        $form->setClassEtiquetas('td_label');

        $form->addEtiqueta(METODOLOGIAS_NOMBRE);
        $form->addInputText('text', 'txt_nombre', 'txt_nombre', '50', '50', $nombre, '', 'onkeypress="ocultarDiv(\'error_nombre\');"');
        $form->addError('error_nombre', ERROR_METODOLOGIAS_NOMBRE);

        $form->addEtiqueta(METODOLOGIAS_ARCHIVO);
        $form->addInputFile('file', 'file_archivo', 'file_archivo', '25', 'file', 'onChange="ocultarDiv(\'error_archivo\');"');
        $form->addError('error_archivo', ERROR_METODOLOGIAS_ARCHIVO);
        ...
        break;
    /**
     * la variable saveAdd, permite almacenar el objeto METODOLOGIAS en la base de datos, ver la clase CManualData
     */
    case 'saveAdd':
        $nombre = $_REQUEST['txt_nombre'];
        ...
        break;
    /**
     * la variable delete, permite hacer la carga del objeto METODOLOGIAS y espera confirmacion de eliminarlo, ver la clase CManual
     */
    case 'delete':
        ...
        echo $html->generaAdvertencia(METODOLOGIAS_MSG_BORRADO, '?mod=' . $modulo . '&niv=1&task=confirmDelete&id_element=' . $id_delete, "cancelarAccion('frm_delete_manual','?mod=" . $modulo . "&niv=1')");

        break;
    /**
     * la variable confirmDelete, permite eliminar el objeto METODOLOGIAS de la base de datos, ver la clase CManualData
     */
    case 'confirmDelete':
        $id_delete = $_REQUEST['id_element'];
        $manual = new CManual($manData);
        ...
        break;
    /**
     * la variable edit, permite hacer la carga del objeto METODOLOGIAS y espera confirmacion de edicion, ver la clase CManual
     */
    case 'edit':
        ...
        $form = new CHtmlForm();
        $form->setTitle(EDITAR_METODOLOGIAS);
        ...
        break;
    /**
     * la variable saveEdit, permite actualizar el objeto METODOLOGIAS en la base de datos, ver la clase CManualData
     */
    case 'saveEdit':
