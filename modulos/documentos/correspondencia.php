...
$daoRespuesta = new CComunicadoRespuestaData($db);
...
switch ($task) {
    ...
    case 'list':
        ...
        $criterio_alarma = "u.usu_id = $id_usuario AND doe.doe_id IN (3,4)";
        $respuestas = $daoRespuesta->getComunicados($criterio_alarma);
        ...
        $titulos = array(CORRESPONDENCIA_AREA, CORRESPONDENCIA_SUBTEMA, CORRESPONDECIA_CODIGO_REFERENCIA,
            ...
            CORRESPONDENCIA_RESPONSABLE_RESPUESTA,CORRESPONDENCIA_ANEXOS, CORRESPONDENCIA_ESTADO);
...
//'onClick=consultar_compromisos();'
    ...
    case 'delete':
        ...
        echo $html->generaAdvertencia(DOCUMENTO_MSG_BORRADO, '?mod=' . $modulo . '&niv=' . $niv . '&task=confirmDelete&id_element=' . $id_delete, "cancelarAccion('frm_delet_correspondencia','?mod=" . $modulo . "&niv=" . $niv . "');");
...
//            $documentoRespuesta = NULL;
    ...
    case 'listAlarmas':
        ...
        $form->setMethod("post");
        $form->writeForm();
        $criterio = "u.usu_id = $id_usuario AND doe.doe_id IN (3,4)";
        $respuestas = $daoRespuesta->getComunicados($criterio);
        $dt = new CHtmlDataTableAlignable();
        $titulos = array(CORRESPONDENCIA_AREA, CORRESPONDENCIA_SUBTEMA, CORRESPONDECIA_CODIGO_REFERENCIA,
            CORRESPONDENCIA_FECHA_RADICADO, CORRESPONDENCIA_DESCRIPCION,
            CORRESPONDENCIA_DOCUMENTO, CORRESPONDENCIA_FECHA_RESPUESTA,
     ...
     */
    case 'saveResponder':
        $idComunicado = $_REQUEST['id_element'];
        $idRespuesta = $_REQUEST['num_consecutivo_respuesta'];

        $comunicadoRespuesta = new CComunicadoRespuesta($idComunicado, $idRespuesta);

        $r = $daoRespuesta->insertComunicadoRespuesta($comunicadoRespuesta);
        $m = ERROR_RESPONDER_COMUNICADO;
        if ($r == 'true') {
            $m = EXITO_RESPONDER_COMUNICADO;
        }
    ...
    case 'see':
        ...
        $respuestas = $daoRespuesta->getRespuestasByComunicado($id_element);
        $dt = new CHtmlDataTableAlignable();
        $titulos = array(CORRESPONDENCIA_AREA, CORRESPONDENCIA_SUBTEMA, CORRESPONDECIA_CODIGO_REFERENCIA,
            CORRESPONDENCIA_FECHA_RADICADO, CORRESPONDENCIA_DESCRIPCION,
            CORRESPONDENCIA_DOCUMENTO, CORRESPONDENCIA_FECHA_RESPUESTA,
