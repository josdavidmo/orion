...
/**
 * Modulo Manuales
 * maneja el modulo DOCUMENTOS_CONTRACTUALES en union con CManual y CManualData
 *
 * @see CManual
 * @see CManualData
 *
...
$manData = new CDocumentosContractualesData($db);
...
switch ($task) {
    /**
     * la variable list, permite hacer la carga la página con la lista de objetos DOCUMENTOS_CONTRACTUALES según los parámetros de entrada
     */
    case 'list':
        $criterio = '1';
        ...
        $titulos = array(DOCUMENTOS_CONTRACTUALES_NOMBRE, DOCUMENTOS_CONTRACTUALES_ARCHIVO);
        $dt->setDataRows($manuales);
        $dt->setTitleRow($titulos);

        $dt->setTitleTable(TABLA_DOCUMENTOS_CONTRACTUALES);
        ...
        break;
    /**
     * la variable add, permite hacer la carga la página con las variables que componen el objeto DOCUMENTOS_CONTRACTUALES, ver la clase CManual
     */
    case 'add':
        ...
        $form->setTitle(AGREGAR_DOCUMENTOS_CONTRACTUALES);
        $form->setId('frm_add_manual');
        $form->setMethod('post');
        $form->setClassEtiquetas('td_label');

        $form->addEtiqueta(DOCUMENTOS_CONTRACTUALES_NOMBRE);
        $form->addInputText('text', 'txt_nombre', 'txt_nombre', '50', '50', $nombre, '', 'onkeypress="ocultarDiv(\'error_nombre\');"');
        $form->addError('error_nombre', ERROR_DOCUMENTOS_CONTRACTUALES_NOMBRE);

        $form->addEtiqueta(DOCUMENTOS_CONTRACTUALES_ARCHIVO);
        $form->addInputFile('file', 'file_archivo', 'file_archivo', '25', 'file', 'onChange="ocultarDiv(\'error_archivo\');"');
        $form->addError('error_archivo', ERROR_DOCUMENTOS_CONTRACTUALES_ARCHIVO);
        ...
        break;
    /**
     * la variable saveAdd, permite almacenar el objeto DOCUMENTOS_CONTRACTUALES en la base de datos, ver la clase CManualData
     */
    case 'saveAdd':
        $nombre = $_REQUEST['txt_nombre'];
        ...
        break;
    /**
     * la variable delete, permite hacer la carga del objeto DOCUMENTOS_CONTRACTUALES y espera confirmacion de eliminarlo, ver la clase CManual
     */
    case 'delete':
        ...
        echo $html->generaAdvertencia(DOCUMENTOS_CONTRACTUALES_MSG_BORRADO, '?mod=' . $modulo . '&niv=1&task=confirmDelete&id_element=' . $id_delete, "cancelarAccion('frm_delete_manual','?mod=" . $modulo . "&niv=1')");

        break;
    /**
     * la variable confirmDelete, permite eliminar el objeto DOCUMENTOS_CONTRACTUALES de la base de datos, ver la clase CManualData
     */
    case 'confirmDelete':
        $id_delete = $_REQUEST['id_element'];
        $manual = new CManual($manData);
        ...
        break;
    /**
     * la variable edit, permite hacer la carga del objeto DOCUMENTOS_CONTRACTUALES y espera confirmacion de edicion, ver la clase CManual
     */
    case 'edit':
        ...
        $form = new CHtmlForm();
        $form->setTitle(EDITAR_DOCUMENTOS_CONTRACTUALES);
        ...
        break;
    /**
     * la variable saveEdit, permite actualizar el objeto DOCUMENTOS_CONTRACTUALES en la base de datos, ver la clase CManualData
     */
    case 'saveEdit':
