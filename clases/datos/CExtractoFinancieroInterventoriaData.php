...
* @subpackage datos
*/
Class CExtractoFinancieroInterventoriaData{
    var $db = null;
	...
	function insertExtractoInterventoria($extracto){
        ...
        function getSaldoInicialByFecha($cuenta,$mes,$anio){
            $sql = "select SUM(mov_valor * t.mov_tipo_factor) as saldo
                    from extracto_movimiento_detalle, extracto_movimiento m, extractos_movimiento_tipo t, extracto_financiero_int 
...
//            echo $sql;
	...
	function getExtractoById($id){
		...
		$sql = "select e.efi_id, c.cfi_id, c.cfi_numero, c.cfi_nombre, 
                        ...
                        from extracto_financiero_int e 
...
//		echo ("<br>sql:".$sql);
        ...
        function getMovimientosByExtracto($extracto){
            $sql="SELECT d.*, m.*, t.* , (d.mov_valor*t.mov_tipo_factor) as valor, mov_fecha as fecha  "
                    . "FROM extracto_movimiento_detalle d , extracto_movimiento m , extractos_movimiento_tipo t "
...
//            echo $sql;
        ...
        function saveMovimiento($extracto,$movimiento,$fecha,$valor){
            $tabla = "extracto_movimiento_detalle";
        ...
        function getValorMovimiento($id_mov){
            $sql="SELECT mov_valor AS mov FROM extracto_movimiento_detalle WHERE mov_detalle_id = $id_mov";
            $r=$this->db->ejecutarConsulta($sql);
            if($r){
			while($w = mysql_fetch_array($r)){
                                return $w['mov'];
        ...
        function getDiaMovimiento($id_mov){
            $sql="SELECT DAY(mov_fecha) AS dia FROM extracto_movimiento_detalle WHERE mov_detalle_id = $id_mov";
            $r=$this->db->ejecutarConsulta($sql);
            if($r){
			while($w = mysql_fetch_array($r)){
                                return $w['dia'];
        ...
        function getTipoMovimiento($id_mov){
            $sql="SELECT mov_movimiento AS mov FROM extracto_movimiento_detalle WHERE mov_detalle_id = $id_mov";
            $r=$this->db->ejecutarConsulta($sql);
            if($r){
			while($w = mysql_fetch_array($r)){
                                return $w['mov'];
        ...
        function getMovimientosGeneral($criterio){
            $sql="SELECT (d.mov_valor*t.mov_tipo_factor) as valor, mov_detalle_id, mov_fecha as fecha, mov_descripcion, c.cfi_numero "
                    . "FROM extracto_movimiento_detalle d , extracto_movimiento m , extractos_movimiento_tipo t, extracto_financiero_int e , cuentas_financiero c "
