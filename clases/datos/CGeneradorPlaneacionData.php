...
class CGeneradorPlaneacionData{
     ...
     * @return Array
     */
    function getInstrumentos($orden) {
        $instrumentos = null;
        $sql = " select * from instrumentos order by " . $orden;
        $r = $this->db->ejecutarConsulta($sql);
        if ($r) {
            $cont = 0;
            while ($w = mysql_fetch_array($r)) {
    ...
    function getEncabezados(){
        $instrumentos = null;
        $sql = " select * from opcion where opn_id = 1 AND opc_variable LIKE 'genEjecucion&ref%' order by opc_nombre";
        $r = $this->db->ejecutarConsulta($sql);
        if ($r) {
            $cont = 0;
            while ($w = mysql_fetch_array($r)) {
...
//        echo $sql;
    ...
    function getPlaneacionById($id) {
        ...
        $sql = "SELECT p.pla_id,  d.der_id,d.dep_id,m.mun_id,e.eje_id, p.pla_numero_encuestas,p.pla_fecha_inicio,p.pla_fecha_fin,p.usu_id "
                . "FROM generador_planeacion p left join eje e on e.eje_id = p.eje_id "
                . "left join municipio m on m.mun_id = p.mun_id "
